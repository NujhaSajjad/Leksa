    ...
    <div style={{ display: "flex", alignItems: "center", gap: 3, height: 14 }}>
      {Array.from({ length: count }).map((_, i) => (
        <div key={i} style={{
          ...
          animation: active ? `bar-bounce ${0.4 + (i % 3) * 0.13}s ease-in-out infinite` : "none",
...
const STORAGE_KEY = "leksa_sessions";
...
const saveSession = (files, sessionId) => {
  ...
  const newSession = {
    ...
    title: files[0]?.name?.replace(/\.[^/.]+$/, "") || "New Session",
    doc: files[0]?.name || "",
    icon: iconFor(files[0]?.name),
    timestamp: Date.now(),
    progress: 5,
  };
  const existing = getSessions();
  const updated = [newSession, ...existing.filter(s => s.id !== newSession.id)].slice(0, 6);
...
function SessionScreen({ onBack, sessionId, uploadedFiles = [] }) {
  ...
  useEffect(() => {
    ...
    ws.onopen = () => { setStatus("speaking"); startMic(ws); };
      ...
      const vad = await MicVAD.new({
        ...
        onSpeechStart: () => {
          ...
          if (geminiGainRef.current)
            geminiGainRef.current.gain.setTargetAtTime(0, audioCtxRef.current.currentTime, 0.015);
        ...
        onSpeechEnd: () => {
          ...
          if (geminiGainRef.current)
            geminiGainRef.current.gain.setTargetAtTime(1.0, audioCtxRef.current.currentTime, 0.05);
          if (ws.readyState === WebSocket.OPEN)
...
/* ─── UPLOAD MODAL ─────────────────────────────── */
function UploadModal({ onClose, onStart, hintDoc }) {
...
function HomeScreen({ onStart, onResume, resumeDoc, onClearResume }) {
  ...
  const handleStart = (files, sid) => {
    const updated = saveSession(files, sid);
    setSessions(updated);
    closeModal();
    onStart(files, sid);
  };
