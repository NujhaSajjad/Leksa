...
function SessionScreen({ onBack, sessionId, uploadedFiles = [] }) {
  ...
  const processorRef = useRef(null); // fallback ke liye
  ...
  // Fallback: purana ScriptProcessor — agar VAD load na ho
  const startMic = async (ws) => {
    try {
      ...
      const vad = await MicVAD.new({
        ...
        onVADMisfire: () => {
          // bahut choti noise thi — ignore, kuch mat bhejo
          setUserSpeaking(false);
        },
      });
      ...
      // VAD package install nahi hua? Fallback to raw ScriptProcessor
      console.warn("VAD unavailable, using raw mic stream:", err.message);
      setVadReady(false);
      try {
        ...
        processor.onaudioprocess = (e) => {
          ...
          ws.send(
            JSON.stringify({
              type: "audio",
              data: toPCM16b64(e.inputBuffer.getChannelData(0)),
            }),
          );
        };
